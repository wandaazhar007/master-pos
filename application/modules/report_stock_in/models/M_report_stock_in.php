...
class M_report_stock_in extends CI_Model
{
  ...
  function _sql()
  {
    $this->db->select("`stock`.`idstock`, `stock`.`date_created`, `stock`.`type`, `stock`.`total`, `stock`.`created_by`, `product`.`idproduct`, `product`.`name`, `product`.`code_product`, `supplier`.`name_supplier`", false);
    $this->db->from("stock");
